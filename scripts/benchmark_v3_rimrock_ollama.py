...
DEFAULT_HOST = "http://172.16.0.248:11434"
DEFAULT_MODEL = "gemma4-e2b:latest"
THINKING = "off"
RUNTIME = "ollama"
...
def chat(host: str, model: str, messages: list[dict[str, str]], max_tokens: int) -> dict[str, Any]:
    ...
    tok_s = round(eval_count / (eval_duration_ns / 1_000_000_000), 2) if eval_count and eval_duration_ns else None
    ttft_ms = round(prompt_eval_duration_ns / 1_000_000, 2) if prompt_eval_duration_ns else None
    return {
...
def run_q8(host: str, model: str) -> dict[str, Any]:
    ...
    turn3 = chat(host, model, messages, 128)
...
def build_report(host: str, model: str, results: list[dict[str, Any]]) -> str:
    ...
    header = [
        "# Gemma 4 E2B — Ollama Benchmark v3 Results",
        "",
        f"Run: `{now}`",
        f"Model: `{model}`",
        "Machine: `rimrock`",
        ...
        f"Host: `{host}`",
        f"Thinking: `{THINKING}`",
        "Request settings: `think=false, temperature=0.1, top_p=0.9, seed=42, repeat_penalty=1.0`",
...
def main() -> None:
    ...
    raw_path = Path("/tmp") / f"benchmark-v3-rimrock-{stamp}.json"
    report_path = Path("/tmp") / f"benchmark-report-gemma4-e2b-ollama-rimrock-v3-{stamp}.md"
